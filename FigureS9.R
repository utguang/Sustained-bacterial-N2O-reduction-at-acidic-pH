...
D.acididurans <- Acididurans_substrat[,c(-4:-8)]
...
Substrate<-Acididurans_substrat
OD_StrainD<-subset(Acididurans_substrat,Strain=="D")
OD_Co<-subset(Acididurans_substrat,Strain=="Co")

OD_plot_D<-ggplot(OD_StrainD,aes(Day,OD))+
...
OD_plot_Co<-ggplot(OD_Co,aes(Day,OD))+
  facet_wrap(~Strain,scales = "free_y",ncol = 1)+  
  geom_errorbar(mapping=aes(ymin=(OD-error),ymax=(OD+error),color=Treatment),width=0.2)+
  geom_point(aes(Day,OD,color=Treatment,shape=Treatment),size=3)+
  geom_line(aes(Day,OD,color=Treatment),size=0.8)+word_mytheme+
...
total<-(OD_plot_Co|OD_plot_D)/plot_spacer()


graph2ppt(x=total,file="FigureS7",margins=c(0,0,0,0),upscale=T,
          append=T,width=14,height=10)


  
save.image(file = "FigureS7_StrainD.Rdata")
