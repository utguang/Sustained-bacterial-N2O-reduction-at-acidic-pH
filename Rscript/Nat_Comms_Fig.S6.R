...
Fig.S6<-ggplot()+
  geom_point(data = Fig_S6,aes(x=Day,y=Peak,color=Treatment),size=2)+
  geom_smooth(data = Fig_S6,aes(x=Day,y=Peak,color=Treatment),linewidth=0.5)+
  word_mytheme+
...
graph2ppt(x=Fig.S6,file="FigureS6",margins=c(0,0,0,0),upscale=T,
