...
N2O<-ggplot(Fig_S5A,aes(Day,Substrate))+
...
rate<-ggplot(Fig_S5B, aes(x = as.character(Treatment), y = rate)) +
...
FigureS5<-(N2O|rate|N_rate)+plot_layout(ncol = 3,widths = c(1,1,1)) 
#combine ABC plots


library(export)
graph2ppt(x=FigureS5,file="FigureS5",margins=c(0,0,0,0),upscale=T,
