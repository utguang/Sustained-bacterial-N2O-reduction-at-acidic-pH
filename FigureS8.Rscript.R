...
cluster<-Exhalaribacter
cluster$orientation<-ifelse(cluster$strand=="+",1,-1)



cluster_gene<-ggplot(cluster,aes(xmin=start,xmax=stop,y=Function,forward=orientation)) +
...
graph2ppt(x=cluster_gene,file="FigureS8",margins=c(0,0,0,0),upscale=T,
          append=T,width=14,height=10)

save.image(file = "FigureS8.Rdata")
